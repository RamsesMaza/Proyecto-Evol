...
async function main() {
  ...
  for (const p of permsData) {
    const perm = await prisma.permission.upsert({
      ...
      create: { name: p.name, slug: p.slug, description: p.description ?? '', module: p.module },
    });
  ...
  for (const perm of createdPerms.filter(p => salesSlugs.includes(p.slug))) {
    await prisma.rolePermission.upsert({
  ...
  // USER gets basic
  const userSlugs = [
    'dashboard.view', 'products.list', 'orders.list', 'orders.create', 'orders.view-detail',
    'tickets.list', 'tickets.create',
  ];
  for (const perm of createdPerms.filter(p => userSlugs.includes(p.slug))) {
    await prisma.rolePermission.upsert({
