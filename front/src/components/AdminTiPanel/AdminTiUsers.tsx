...
const AdminTiUsers = () => {
  ...
  const roleIcon = (r: string) => {
    switch (r) {
      ...
      case 'TI': return <FaUserCog />;
      case 'SALES': return <FaUserTag />;
  ...
  return (
    <div className={styles.module}>
      <div className={styles.toastContainer}>
        {actionMsg && <div className={`${styles.toast} ${actionMsg.includes('Error') ? styles.toastError : ''}`}><span className={styles.toastIcon}>{actionMsg.includes('Error') ? <FaExclamationTriangle /> : <FaCheckCircle />}</span> {actionMsg}</div>}
      </div>
      ...
      <div className={styles.statsRow}>
        {statCards.map((s, i) => (
          <div key={i} className={styles.statCard} style={{ '--stat-color': s.color } as React.CSSProperties}>
      ...
      <div className={styles.filterBar}>
        <div className={styles.searchWrap}>
          ...
          <input className={styles.searchInput} value={query} onChange={e => { setQuery(e.target.value); setPage(0); }} placeholder="Buscar por nombre, email..." />
      ...
      <div className={styles.tableCard}>
        {loading ? (
          <div className={styles.loading}><FaSpinner className={styles.spinner} /> Cargando usuarios...</div>
        ) : (
                    ...
                    <tr key={u.id} className={styles.tr}>
                      <td>
                        <div className={styles.userCell}>
                          <div className={styles.userAvatar} style={{ background: `${rc.text}14`, color: rc.text }}>
                      ...
                      <td>
                        <span className={styles.roleBadge} style={{ background: rc.bg, color: rc.text }}>
                          {roleIcon(u.role)} {u.role}
                        </span>
                      </td>
                      <td>
                        <span className={styles.statusBadge} style={{ background: `${statusColor(u.status)}14`, color: statusColor(u.status) }}>
                          {u.status === 'activo' ? <FaCheckCircle /> : <FaTimesCircle />} {u.status}
                        </span>
                      </td>
                      <td>
                        <span className={styles.dateCell}><FaCalendarAlt /> {new Date(u.createdAt).toLocaleDateString('es-PE', { day: '2-digit', month: 'short', year: 'numeric' })}</span>
                      </td>
                      <td className={styles.tdActions}>
                        <div className={styles.actionsWrap}>
                          ...
                          <div className={styles.actionDropdown}>
                            <button className={styles.actionBtn} onClick={() => setOpenMenuId(openMenuId === u.id ? null : u.id)}><FaEllipsisV /></button>
                            {openMenuId === u.id && (
                              <div className={styles.dropdownMenu}>
                                <button onClick={() => { setOpenMenuId(null); u.status !== 'activo' && handleStatusChange(u.id, 'activo'); }}>
                                  <FaToggleOn /> Activar
                                </button>
                                <button onClick={() => { setOpenMenuId(null); handleStatusChange(u.id, 'inactivo'); }}>
                                  <FaToggleOff /> Desactivar
                                </button>
                                <button onClick={() => { setOpenMenuId(null); handleStatusChange(u.id, 'bloqueado'); }}>
                                  <FaLock /> Bloquear
                                </button>
                                <div className={styles.dropdownDivider} />
                                {ROLES.filter(r => r !== u.role).map(r => (
                                  <button key={r} onClick={() => { setOpenMenuId(null); handleRoleChange(u.id, r); }}>
                                    <FaUserTag /> Cambiar a {r}
                                  </button>
                                ))}
                                <div className={styles.dropdownDivider} />
                                <button onClick={() => { setOpenMenuId(null); setNewPassword(''); setModal({ type: 'password', user: u }); }}>
                                  <FaKey /> Reset Password
                                </button>
                                <button className={styles.dropdownDanger} onClick={() => { setOpenMenuId(null); setConfirmDelete(u); }}>
                                  <FaTrashAlt /> Eliminar
                                </button>
                              </div>
                            )}
        ...
        <div className={styles.modalOverlay} onClick={() => setModal(null)}>
          <div className={styles.modal} onClick={e => e.stopPropagation()}>
            ...
            <div className={styles.modalBody}>
              <div className={styles.formGrid}>
                <div className={styles.field}>
                  <label>Nombre</label>
                  <input value={formData.firstName} onChange={e => setFormData(p => ({ ...p, firstName: e.target.value }))} placeholder="Ej: Juan" />
                </div>
                <div className={styles.field}>
                  <label>Apellidos</label>
                  <input value={formData.lastName} onChange={e => setFormData(p => ({ ...p, lastName: e.target.value }))} placeholder="Ej: Pérez" />
                </div>
                <div className={styles.field}>
                  <label>Email</label>
                  <input type="email" value={formData.email} onChange={e => setFormData(p => ({ ...p, email: e.target.value }))} placeholder="correo@ejemplo.com" />
                </div>
                <div className={styles.field}>
                  <label>Contraseña</label>
                  <input type="password" value={formData.password} onChange={e => setFormData(p => ({ ...p, password: e.target.value }))} placeholder="Mín. 8 caracteres" />
                </div>
                <div className={styles.field}>
                  <label>Teléfono</label>
                  <input value={formData.phone} onChange={e => setFormData(p => ({ ...p, phone: e.target.value }))} placeholder="+51 999 999 999" />
                </div>
                <div className={styles.field}>
                  <label>Empresa</label>
                  <input value={formData.company} onChange={e => setFormData(p => ({ ...p, company: e.target.value }))} placeholder="Nombre de empresa" />
                </div>
                <div className={styles.field}>
                  <label>Rol</label>
            ...
            <div className={styles.modalFooter}>
              ...
              <button className={styles.btnPrimary} onClick={handleCreate} disabled={saving}>{saving ? <FaSpinner className={styles.spinner} /> : 'Crear Usuario'}</button>
            </div>
          </div>
        </div>
      )}
      ...
      {modal?.type === 'edit' && (
        <div className={styles.modalOverlay} onClick={() => setModal(null)}>
          <div className={styles.modal} onClick={e => e.stopPropagation()}>
            ...
            <div className={styles.modalFooter}>
              <button className={styles.btnOutline} onClick={() => setModal(null)}>Cancelar</button>
              <button className={styles.btnPrimary} onClick={handleEdit} disabled={saving}>{saving ? <FaSpinner className={styles.spinner} /> : 'Guardar Cambios'}</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div className={styles.modal} onClick={e => e.stopPropagation()}>
            ...
            <div className={styles.modalBody}>
              <div className={styles.passwordInfo}>
                <FaUserCircle /> {modal.user?.firstName} {modal.user?.lastName} — <strong>{modal.user?.email}</strong>
              </div>
              <div className={styles.field}>
      ...
      {confirmDelete && (
        <div className={styles.modalOverlay} onClick={() => !saving && setConfirmDelete(null)}>
          <div className={styles.modal} onClick={e => e.stopPropagation()} style={{ maxWidth: 420 }}>
            <div className={styles.modalHeader}>
              <h3><FaTrashAlt style={{ color: '#dc2626' }} /> Confirmar Eliminación</h3>
              <button onClick={() => !saving && setConfirmDelete(null)} disabled={saving}>✕</button>
            </div>
            <div className={styles.modalBody}>
              <div className={styles.deleteWarning}>
                <FaExclamationTriangle />
                <p>¿Estás seguro de eliminar permanentemente a <strong>{confirmDelete.firstName} {confirmDelete.lastName}</strong>?</p>
              </div>
              <p className={styles.deleteHint}>Esta acción no se puede deshacer. Todas las sesiones del usuario serán cerradas.</p>
            </div>
            <div className={styles.modalFooter}>
              ...
              <button className={styles.btnDanger} onClick={handleDelete} disabled={saving}>
                {saving ? <><FaSpinner className={styles.spinner} /> Eliminando...</> : 'Eliminar Permanentemente'}
              </button>
            </div>
          </div>
        </div>
          ...
          <div className={`${styles.modal} ${styles.modalWide}`} onClick={e => e.stopPropagation()}>
            ...
            <div className={styles.modalBody}>
              <div className={styles.detailHeader}>
                <div className={styles.detailAvatar} style={{ background: `${roleColor(detail.role).text}14`, color: roleColor(detail.role).text }}>
                  {detail.firstName.charAt(0)}{detail.lastName.charAt(0)}
                </div>
                <div>
                  <h3>{detail.firstName} {detail.lastName}</h3>
                  <div className={styles.detailBadges}>
                    <span className={styles.roleBadge} style={{ background: roleColor(detail.role).bg, color: roleColor(detail.role).text }}>
                      {roleIcon(detail.role)} {detail.role}
                    </span>
                    <span className={styles.statusBadge} style={{ background: `${statusColor(detail.status)}14`, color: statusColor(detail.status) }}>
                      {detail.status}
                    </span>
                  </div>
                </div>
              ...
              <div className={styles.detailGrid}>
                ...
                <div className={styles.detailField}>
                  <FaPhone /> <span>Teléfono</span>
                  <strong>{detail.phone || '—'}</strong>
                </div>
                <div className={styles.detailField}>
                  <FaBuilding /> <span>Empresa</span>
                  <strong>{detail.company || '—'}</strong>
                </div>
                <div className={styles.detailField}>
                  <FaCalendarAlt /> <span>Registro</span>
                  <strong>{new Date(detail.createdAt).toLocaleDateString('es-PE', { day: '2-digit', month: 'long', year: 'numeric' })}</strong>
                </div>
              </div>
                ...
                {activity.slice(0, 8).map((a: any) => (
                  <div key={a.id} className={styles.activityItem}>
                    ...
                    <div>
                      ...
                      <p>{a.description} — {new Date(a.createdAt).toLocaleString('es-PE')}</p>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className={styles.activityList}>
                ...
                {loginHistory.slice(0, 8).map((a: any) => (
                  <div key={a.id} className={styles.activityItem}>
                    <div className={styles.activityDot} style={{ background: a.success ? '#10b981' : '#ef4444' }} />
                    <div>
                      <strong>{a.success ? 'Exitoso' : 'Fallido'}</strong>
                      <p>{new Date(a.createdAt).toLocaleString('es-PE')} — IP: {a.ipAddress || 'N/A'}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className={styles.modalFooter}>
              <button className={styles.btnOutline} onClick={() => setDetail(null)}>Cerrar</button>
            </div>
          </div>
        </div>
      )}
